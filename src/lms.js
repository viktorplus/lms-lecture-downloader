...
async function getCourseActivities(page) {
  ...
  return page.evaluate((downloadableTypes) => {
    function cleanName(raw) {
      ...
      // Срезать статус проверки задания (его нет в эталонных именах). Делаем
      // это ПО КЛЮЧЕВЫМ СЛОВАМ статуса, не завязываясь на конкретный глиф
      // тире/галочки: находим статус и отрезаем его вместе с предшествующими
      // разделителями. Примеры: «✓ Зачёт», «— не проверено», «требует оценки».
      // Без \b: в JS он ASCII-ориентирован и не срабатывает на кириллице.
      // Поэтому требуем хотя бы один разделитель/символ статуса перед словом.
      s = s.replace(
        /[\s—–\-−·•|✓✔✗✕☑*]+(не\s*проверено|проверено|требует\s*оценки|сдано|не\s*сдано|просрочено|ожидает(?:\s*оценки)?|неза[чн]ё?т|за[чн]ё?т).*$/iu,
        ''
      );
      // Если .instancename не было и взяли textContent, в хвосте может остаться
      // accesshide-метка типа активности — её нет в эталонных именах.
      s = s.replace(/\s+(Задание|Файл|Страница|Папка|Тест|Гиперссылка)\s*$/u, '');
      // Хвостовые разделители/символы статуса, если что-то осталось.
      s = s.replace(/[\s—–\-−·•|✓✔✗✕☑*]+$/u, '');
      return s.trim();
    }
